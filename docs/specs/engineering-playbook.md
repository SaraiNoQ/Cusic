...
1. 托管 LLM API
2. 小米 MiMo TTS (mimo-v2.5-tts, VoiceDesign, VoiceClone) — 主要语音合成提供商
3. 阿里云 ASR (Aliyun ASR) — 语音识别，备用
4. 阿里云 TTS (Aliyun TTS) — 语音合成，备用
5. 外部搜索 Provider
6. 内容源 Provider
7. 飞书、日历、天气等上下文 Provider
8. `nestjs-pino`（JSON 结构化日志）
9. `@nestjs/throttler`（速率限制）
10. `helmet`（安全头）
11. `compression`（gzip 压缩）
...
2. 私密变量不允许出现在前端构建链路
3. `.env.example` 必须维护
4. 启动时通过 `validateEnv()`（`apps/api/src/common/env-validation.ts`）对环境变量做强制性校验：校验 `DATABASE_URL`、`REDIS_URL`、JWT 密钥等关键配置是否存在，并对占位值（如 `CHANGE_ME`）发出警告，防止因配置缺失导致运行时异常。
...
5. CusicLogo — SVG logo with gold metallic gradient, arc-based C + serif USIC text, grain texture overlay
...
- 日志层：使用 `nestjs-pino` 替代 NestJS 默认 Logger，输出 JSON 结构化日志，便于日志聚合与检索。
- 异常层：`GlobalExceptionFilter`（`apps/api/src/common/global-exception.filter.ts`）统一捕获所有异常，输出一致的错误 envelope。
- 链路追踪：`RequestIdInterceptor`（`apps/api/src/common/request-id.ts`）为每个请求注入 `requestId`，贯穿日志与错误响应。
...
3. 端口、卷、网络必须有明确命名。
4. 不直接在宿主机裸跑 Node 服务。
5. 所有 5 个服务（web、api、worker、postgres、redis）均配置 Docker HEALTHCHECK 指令。
6. 服务间依赖使用 `condition: service_healthy` 确保启动顺序正确。
7. 镜像构建支持 `IMAGE_TAG` 环境变量，用于区分版本和回滚。
...
4. 数据目录和日志目录已规划
5. 关键配置文件已备份
6. `MIMO_API_KEY` 用于小米 MiMo TTS 语音合成
7. API 运行时依赖已包含：`compression`、`helmet`、`@nestjs/throttler`、`nestjs-pino`
8. CI/CD 通过 GitHub Actions 自动执行 lint、typecheck、构建与部署流程
...
1. Storybook 或组件文档站
2. 更完整的监控体系
3. ~~自动化数据库备份~~ **Done** — 参见 `scripts/backup-db.sh`、`scripts/setup-backup-cron.sh`
4. ~~灰度发布和回滚脚本~~ **Done** — 参见 `scripts/restore-db.sh`，`IMAGE_TAG` 支持版本回滚
5. 更完整的 E2E 测试
6. 公网发布安全加固
